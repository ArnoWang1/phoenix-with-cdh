...
import com.nan.custom.CDHMRJobConfig;
...
public class PhoenixMRJobUtil {
    ...
    public static void updateCapacityQueueInfo(Configuration conf) {
        conf.set(CDHMRJobConfig.QUEUE_NAME,
        ...
        conf.setInt(CDHMRJobConfig.MAP_MEMORY_MB, mapMemoryMB);
        conf.set(CDHMRJobConfig.MAP_JAVA_OPTS, XMX_OPT + ((int) (mapMemoryMB * 0.9)) + "m");

        LOG.info("Queue Name=" + conf.get(CDHMRJobConfig.QUEUE_NAME) + ";" + "Map Meory MB="
                + conf.get(CDHMRJobConfig.MAP_MEMORY_MB) + ";" + "Map Java Opts="
                + conf.get(CDHMRJobConfig.MAP_JAVA_OPTS));
    }
}
